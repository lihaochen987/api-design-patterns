...
public abstract class UpdateInventoryHandlerTestBase
{
    protected Fixture Fixture = new();
    protected readonly InventoryRepositoryFake Repository = [];

    protected ICommandHandler<UpdateInventoryCommand> UpdateInventoryService()
    {
        return new UpdateInventoryHandler(Repository);
    }
}
